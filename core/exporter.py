...
def export_transfer_batch(
    ...
    if not adapter_name:
        ...
        if not adapter_name:
            logger.error(f"Cannot export: Unsupported file extension '{ext}'. Supported: {list(SUPPORTED_EXPORT_FORMATS.keys())}")
            return False
    ...
    if adapter_name not in available_adapters:
         logger.error(f"Cannot export: OTIO adapter '{adapter_name}' is not available for writing in this environment. Available: {available_adapters}")
         return False
    elif adapter_name not in SUPPORTED_EXPORT_FORMATS.values():
         logger.warning(f"Exporting using potentially unsupported adapter '{adapter_name}'. Known supported: {list(SUPPORTED_EXPORT_FORMATS.values())}")
    ...
    for segment in transfer_batch.segments:
        if segment.original_source and segment.original_source.frame_rate:
            timeline_rate = segment.original_source.frame_rate; break
        elif segment.transfer_source_range:
            timeline_rate = segment.transfer_source_range.duration.rate; break
    ...
    if norm_sep_frames > 0:
        try:
             ...
             logger.error(f"Could not create separator gap object (duration: {norm_sep_frames} frames @ {timeline_rate}fps): {e}")
    ...
    for i, segment in enumerate(transfer_batch.segments):
        ...
        try:
             ...
             logger.error(f"Skipping segment {i+1}: Could not create file URL for path '{original_source.path}': {url_err}.")
             continue
        ...
        if segment.source_edit_shots:
             ...
             if first_shot.edit_metadata:
                 possible_keys = ["Tape Name", "Reel Name", "TapeID", "Reel", "reel", "source_clip", "reel id"] # More keys
        ...
        if adapter_name == 'cmx_3600_edl':
            tape_name = ''.join(c if c.isalnum() else '_' for c in tape_name.upper())[:8] # Keep only alphanum/_, uppercase, max 8
        ...
        try:
             ...
             added_clip_count += 1
             logger.debug(f"Appended clip '{otio_clip.name}' to track. Source: {os.path.basename(original_source.path)}, Range: {transfer_range}")
    ...
    logger.info(f"Writing {added_clip_count} clips (with gaps: {separator_gap is not None}) to '{abs_output_path}' using adapter '{adapter_name}'...")
    try:
