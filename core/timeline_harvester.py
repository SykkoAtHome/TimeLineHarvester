...
def time_from_json(json_data: Optional[Union[List, Dict]]) -> Optional[Union[otio.opentime.RationalTime, otio.opentime.TimeRange]]:
    ...
    if isinstance(json_data, list) and len(json_data) == 2:
        try: return opentime.RationalTime(value=json_data[0], rate=json_data[1])
        except: return None # Handle errors gracefully
...
class TimelineHarvester:
    ...
    def parse_added_edit_files(self) -> bool:
        ...
        logger.info(f"Parsing complete. Parsed {successful_parses}/{len(self.edit_files)} files. Found {total_shots_parsed} EditShots.")
    ...
    def find_original_sources(self) -> Tuple[int, int, int]:
        ...
        logger.info(f"Original source lookup finished. Processed: {total_processed}. Found: {found_count}, Not Found: {not_found_count}, Errors: {error_count}")
    ...
    def set_output_profiles(self, profiles_config: List[Dict]):
        ...
        for config in profiles_config:
            try:
                ...
                else: logger.warning(f"Skipping invalid profile config: {config}")
            except Exception as e: logger.warning(f"Skipping invalid profile config {config}: {e}")
    ...
    def set_color_prep_handles(self, start_handles: int, end_handles: int):
         ...
         if norm_end != self.color_prep_end_handles:
              ...
              changed = True
         if changed: logger.info(f"Set color prep handles: Start={self.color_prep_start_handles}, End={self.color_prep_end_handles}")
    ...
    def calculate_transfer(self, stage: str): # Removed handle/dir args, use internal state
        ...
        if stage == 'color': self.color_transfer_batch = None
        elif stage == 'online': self.online_transfer_batch = None
        else: logger.error(f"Unknown stage '{stage}'."); return
        ...
        else: return # Already handled

        if not shots_to_process:
            logger.warning(f"[{stage}] No valid shots found to calculate segments.")
            batch = TransferBatch(handle_frames=handles_to_use, output_directory=output_dir_for_stage, batch_name=batch_name)
            ...
            try:
                ...
                batch.unresolved_shots.extend([s for s in self.edit_shots if s.lookup_status != 'found' and s not in batch.unresolved_shots])
                ...
                batch = TransferBatch(handle_frames=handles_to_use, output_directory=output_dir_for_stage, batch_name=f"{batch_name}_Error")
                batch.calculation_errors.append(f"Fatal calculation error: {str(e)}")
                batch.unresolved_shots = self.edit_shots

        # Store the result
        if stage == 'color': self.color_transfer_batch = batch
        elif stage == 'online': self.online_transfer_batch = batch
        ...
        if batch.calculation_errors: logger.warning(log_msg)
            ...
            def serialize_batch(batch: Optional[TransferBatch]) -> Optional[Dict]:
                ...
                for seg in batch.segments:
                    ...
                    for s_shot in seg.source_edit_shots:
                        try: covered_indices.append(edit_shots_list.index(s_shot))
                        except ValueError: pass # Ignore shots not found in main list
                ...
                for s_shot in batch.unresolved_shots:
                     try: unresolved_indices.append(edit_shots_list.index(s_shot))
    ...
    def load_project(self, file_path: str) -> bool:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f: project_data = json.load(f)
            ...
            self.edit_files = [EditFileMetadata(path=f_data['path'], format_type=f_data.get('format'))
                               for f_data in project_data.get("edit_files", []) if isinstance(f_data, dict) and 'path' in f_data]
            ...
            for path, source_data in sources_cache_data.items():
                 if isinstance(source_data, dict):
                     ...
                     else: logger.warning(f"Skipping invalid source cache entry for {path}")
            ...
            for i, shot_data in enumerate(edit_shots_data):
                 if isinstance(shot_data, dict):
                     ...
                     found_original = self.original_sources_cache.get(original_source_path) if original_source_path else None
                     edit_range = time_from_json(shot_data.get("edit_media_range"))
                     timeline_range = time_from_json(shot_data.get("timeline_range"))
                     if not isinstance(edit_range, opentime.TimeRange): logger.warning(f"Skipping edit shot due to invalid range data: {shot_data}"); continue
            ...
            def deserialize_batch(batch_data: Optional[Dict], stage: str) -> Optional[TransferBatch]:
                 ...
                 handles = batch_data.get("handle_frames", self.color_prep_start_handles if stage=='color' else self.online_prep_handles)
                 output_dir = batch_data.get("output_directory", self.online_output_directory if stage=='online' else None)
                 ...
                 for i, seg_data in enumerate(serialized_segments):
                     if isinstance(seg_data, dict):
                         ...
                         if not original_source or not isinstance(transfer_range, opentime.TimeRange): logger.warning(...); continue

                         covered_shots = [temp_edit_shots_map.get(idx) for idx in seg_data.get("source_edit_shots_indices", []) if idx in temp_edit_shots_map]

                         batch.segments.append(TransferSegment(
                             original_source=original_source, transfer_source_range=transfer_range,
                             output_targets=seg_data.get("output_targets", {}), status=seg_data.get("status", "calculated"),
                             error_message=seg_data.get("error_message"), source_edit_shots=covered_shots ))
                 batch.unresolved_shots = [temp_edit_shots_map.get(idx) for idx in unresolved_indices if idx in temp_edit_shots_map]
                 return batch

            # Deserialize Batches
            self.color_transfer_batch = deserialize_batch(project_data.get("color_prep_results", {}).get("transfer_batch"), 'color')
            self.online_transfer_batch = deserialize_batch(project_data.get("online_prep_results", {}).get("transfer_batch"), 'online')

            logger.info(f"Project '{self.project_name}' loaded successfully.")
            return True

        except json.JSONDecodeError as e: logger.error(f"Failed parsing project JSON '{file_path}': {e}"); return False
        except KeyError as e: logger.error(f"Missing key loading project '{file_path}': {e}"); return False
        except Exception as e: logger.error(f"Failed loading project '{file_path}': {e}", exc_info=True); return False
    ...
    def set_color_prep_handles(self, start_handles: int, end_handles: int):
         ...
         if changed: logger.info(f"Set color handles: Start={self.color_prep_start_handles}, End={self.color_prep_end_handles}")

    def set_color_prep_separator(self, separator: int):
         # ... (implementation as before) ...
         norm_sep = max(0, int(separator))
         if norm_sep != self.color_prep_separator: self.color_prep_separator = norm_sep; logger.info(f"Set color separator: {self.color_prep_separator}f")
    ...
    def get_transfer_segments_summary(self, stage='color') -> List[Dict]:
        ...
        for i, seg in enumerate(batch.segments):
             ...
             if seg.transfer_source_range and rate:
                 ...
                 try: tc_string = seg.transfer_source_range.start_time.to_timecode(rate=rate)
                 except: tc_string = f"{seg.transfer_source_range.start_time.to_seconds():.3f}s"
             summary.append({
